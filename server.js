...
function processScrapedListings(term, scraped, jobType) {
  // scan_all: compute average, store, do NOT save history
  if (jobType === 'scan_all') {
...
function processQueue() {
  ...
  // find first job whose term is not active
  let jobIndex = -1;
...
// ─── Embedded Frontend (HTML as a string without template literal issues) ──
...
'</div>\n' +
'<script>\n' +
'const API_BASE=window.location.origin;let bargains={};\n' +
'document.querySelectorAll(".tab").forEach(t=>{t.addEventListener("click",()=>{document.querySelectorAll(".tab").forEach(t=>t.classList.remove("active"));document.querySelectorAll(".tab-content").forEach(t=>t.classList.remove("active"));t.classList.add("active");document.getElementById("tab-"+t.dataset.tab).classList.add("active");});});\n' +
'const ws=new WebSocket("ws://"+window.location.host);ws.onopen=()=>ws.send(JSON.stringify({type:"register-frontend"}));ws.onmessage=e=>{const d=JSON.parse(e.data);if(d.type==="bargain-alert"){addLog("Bargain found for \\""+d.term+"\\": "+d.bargains.length+" items","bargain");renderAll(d);}else if(d.type==="update"){renderAll(d);}};\n' +
'function renderAll(data){renderTerms(data.terms);renderWorkers(data.clients,data.active);if(data.bargains){bargains=data.bargains;renderBargains(data.bargains);const total=Object.values(data.bargains).reduce((s,a)=>s+a.length,0);document.getElementById("bargainCount").textContent=total;}}\n' +
'function renderTerms(terms){const c=document.getElementById("termContainer");if(!terms||terms.length===0){c.innerHTML="<div class=\\"empty\\">No search terms. Add one above.</div>";return;}let html="<table><thead><tr><th>Term</th><th>Avg Price</th><th>Deal %</th><th>Interval</th><th>Status</th><th>Listings</th><th>Bargains</th><th>Actions</th></tr></thead><tbody>";terms.forEach(t=>{const status=t.active?"<span class=\\"badge badge-active\\">Scanning</span>":"<span class=\\"badge badge-idle\\">Idle</span>";const avgDisplay=t.averagePrice?"£"+t.averagePrice:"Not set";let scanBtn="";if(t.averagePrice){if(t.scanning){scanBtn="<button class=\\"danger\\" data-action=\\"stop\\" data-term=\\""+t.term+"\\">Stop Scanning</button>";}else{scanBtn="<button class=\\"success\\" data-action=\\"scan\\" data-term=\\""+t.term+"\\">Start Scanning</button>";}}else{scanBtn="<button class=\\"secondary\\" data-action=\\"avg\\" data-term=\\""+t.term+"\\">Calc Average</button>";}html+="<tr><td><strong>"+t.term+"</strong></td><td>"+avgDisplay+"</td><td>"+t.thresholdPercent+"%</td><td>"+t.interval+" min</td><td>"+status+"</td><td>"+(t.listingCount||0)+"</td><td>"+(t.bargainCount||0)+"</td><td class=\\"inline-actions\\">"+scanBtn+"<button class=\\"danger\\" data-action=\\"remove\\" data-term=\\""+t.term+"\\">Remove</button></td></tr>";});html+="</tbody></table>";c.innerHTML=html;\n' +
'c.querySelectorAll("[data-action=\\"remove\\"]").forEach(btn=>{btn.addEventListener("click",async()=>{const term=btn.dataset.term;if(!confirm("Remove \\""+term+"\\"?"))return;await fetch(API_BASE+"/terms/"+encodeURIComponent(term),{method:"DELETE"});addLog("Removed term: \\""+term+"\\"","warning");fetchData();});});\n' +
'c.querySelectorAll("[data-action=\\"avg\\"]").forEach(btn=>{btn.addEventListener("click",async()=>{const term=btn.dataset.term;const res=await fetch(API_BASE+"/calculate-average",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({term})});const data=await res.json();if(data.success){addLog("Average calculation started for \\""+term+"\\"","info");fetchData();}else{alert(data.error||"Failed to start calculation");}});});\n' +
'c.querySelectorAll("[data-action=\\"scan\\"]").forEach(btn=>{btn.addEventListener("click",async()=>{const term=btn.dataset.term;const res=await fetch(API_BASE+"/start-scan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({term})});const data=await res.json();if(data.success){addLog("Scanning started for \\""+term+"\\"","success");fetchData();}else{alert(data.error||"Failed to start scanning");}});});\n' +
'c.querySelectorAll("[data-action=\\"stop\\"]").forEach(btn=>{btn.addEventListener("click",async()=>{const term=btn.dataset.term;const res=await fetch(API_BASE+"/stop-scan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({term})});const data=await res.json();if(data.success){addLog("Scanning stopped for \\""+term+"\\"","warning");fetchData();}else{alert(data.error||"Failed to stop scanning");}});});}\n' +
'function renderWorkers(clientCount,activeJobs){const c=document.getElementById("workersContainer");if(clientCount===0){c.innerHTML="<div class=\\"empty\\">No workers connected.</div>";return;}let html="<div class=\\"stats-grid\\"><div class=\\"stat-box\\"><div class=\\"value\\">"+clientCount+"</div><div class=\\"label\\">Connected Workers</div></div><div class=\\"stat-box\\"><div class=\\"value\\">"+(activeJobs?activeJobs.length:0)+"</div><div class=\\"label\\">Active Jobs</div></div></div>";if(activeJobs&&activeJobs.length>0){html+="<table><thead><tr><th>Term</th><th>Type</th></tr></thead><tbody>";activeJobs.forEach(term=>{html+="<tr><td>"+term+"</td><td>processing</td></tr>";});html+="</tbody></table>";}c.innerHTML=html;}\n' +
'function renderBargains(b){const c=document.getElementById("bargainContainer");let html="";let has=false;for(const[term,items]of Object.entries(b)){if(items&&items.length>0){has=true;html+="<h4>"+term+" <span class=\\"badge badge-bargain\\">"+items.length+" bargains</span></h4>";items.slice(0,20).forEach(item=>{html+="<div class=\\"bargain-item\\"><strong>"+item.name+"</strong><div style=\\"display:flex;gap:15px;flex-wrap:wrap;font-size:14px;\\"><span>Price: "+item.price+"</span><span>Discount: "+item.discount+"%</span><span>Size: "+(item.size||"N/A")+"</span><span>Condition: "+(item.condition||"N/A")+"</span><a href=\\""+item.link+"\\" target=\\"_blank\\">View</a></div></div>";});if(items.length>20)html+="<p>... and "+(items.length-20)+" more</p>";}}c.innerHTML=has?html:"<div class=\\"empty\\">No bargains yet.</div>";}\n' +
'function addLog(msg,type){if(!type)type="info";const c=document.getElementById("logContainer");const time=new Date().toLocaleTimeString();const d=document.createElement("div");d.innerHTML="<span class=\\"timestamp\\">["+time+"]</span> <span class=\\""+type+"\\">"+msg+"</span>";c.prepend(d);if(c.children.length>100)c.removeChild(c.lastChild);}\n' +
'async function fetchData(){try{const res=await fetch(API_BASE+"/status");const data=await res.json();renderAll(data);}catch(e){console.error(e);}}\n' +
'document.getElementById("addBtn").addEventListener("click",async()=>{const termInput=document.getElementById("newTerm");const thresholdInput=document.getElementById("threshold");const intervalSelect=document.getElementById("interval");const term=termInput.value.trim();const threshold=parseInt(thresholdInput.value)||20;const interval=parseInt(intervalSelect.value)||5;if(!term)return;try{const res=await fetch(API_BASE+"/terms",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({term,thresholdPercent:threshold,interval})});if(res.ok){addLog("Added term: \\""+term+"\\"","success");termInput.value="";fetchData();}else{const err=await res.json();alert(err.error||"Failed to add term");}}catch(err){alert("Network error");}});\n' +
'setInterval(fetchData,3000);fetchData();addLog("Dashboard ready","info");\n' +
'</script>\n' +
'</body>\n' +
'</html>';
...
app.post('/stop-scan', (req, res) => {
  ...
  saveTerms();
  // remove any queued scan_new jobs for this term
  jobQueue = jobQueue.filter(j => !(j.term === term && j.type === 'scan_new'));
  broadcastUpdate();
  res.json({ success: true });
});
...
setInterval(() => {
  for (const termObj of searchTerms) {
    if (!termObj.averagePrice || !termObj.scanning) continue;
    const term = termObj.term;
