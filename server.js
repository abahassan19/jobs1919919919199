...
// ─── State ──────────────────────────────────────────────────────────────
let searchTerms = [];                     // list of term objects: { term, averagePrice, thresholdPercent, interval }
let clients = new Map();                 // clientId -> { ws, busy: false, lastPing: Date }
let jobQueue = [];                       // terms that need scraping
let activeJobs = new Map();              // term -> { clientId, startTime }
let scrapedHistory = {};                 // term -> { listings: [], unique: [] }
let bargains = {};                       // term -> [ listing ]
...
function analyzePrices(listings, averagePrice, thresholdPercent) {
  ...
  const threshold = thresholdPercent || 20; // default 20%
...
function processQueue() {
  ...
  activeJobs.set(term, { clientId: availableClient, startTime: Date.now() });
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    button { padding: 8px 16px; background: #4a6cf7; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 500; }
    ...
    .tab { padding: 10px 20px; cursor: pointer; border: none; background: none; font-weight: 500; color: #666; }
...
<div class="container">
  ...
  <div class="card">
    ...
    <div class="flex">
      ...
      <input type="number" id="avgPrice" placeholder="Avg price (GBP)" step="0.01" style="width:150px;" />
      <input type="number" id="threshold" placeholder="Deal %" value="20" step="1" style="width:120px;" />
      <input type="number" id="interval" placeholder="Minutes" value="5" step="1" style="width:120px;" />
  ...
  <div class="tabs">
    ...
    <button class="tab" data-tab="bargains">💎 Bargains <span id="bargainCount" class="badge" style="background:#dc3545;color:white;">0</span></button>
  ...
  <div id="tab-terms" class="tab-content active">
    <div class="card">
      ...
      <table id="termTable">
        <thead>
          <tr><th>Term</th><th>Avg Price</th><th>Deal %</th><th>Interval</th><th>Status</th><th>Listings</th><th>Bargains</th><th>Actions</th></tr>
        </thead>
  ...
  <div id="tab-bargains" class="tab-content">
    <div class="card">
      ...
      <div id="bargainList">No bargains found yet.</div>
    </div>
  </div>
  ...
  <div id="tab-workers" class="tab-content">
    <div class="card">
      ...
      <div id="workersList">No workers connected.</div>
    </div>
  </div>
...
<script>
  ...
  ws.onmessage = (event) => {
    ...
    if (data.type === 'bargain-alert') {
      addLog(\`💰 NEW BARGAIN for "\${data.term}": \${data.bargains.length} items!\`, 'bargain');
      fetchData();
  ...
  function renderTerms(terms) {
    ...
    if (!terms || terms.length === 0) {
      tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:#999;">No terms configured</td></tr>';
      return;
    }
    terms.forEach(t => {
      const tr = document.createElement('tr');
      const isActive = t.active || false;
      const status = isActive ? '<span class="badge badge-active">Scraping</span>' : '<span class="badge badge-idle">Idle</span>';
      tr.innerHTML = \`
        ...
        <td><button class="danger" data-term="\${t.term}">Remove</button></td>
      \`;
  ...
  function renderBargains(bargainsData) {
    const container = document.getElementById('bargainList');
    let html = '';
    let hasBargains = false;
    for (const [term, items] of Object.entries(bargainsData)) {
      if (items && items.length > 0) {
        hasBargains = true;
        html += \`<h3>\${term} (\${items.length} bargains)</h3>\`;
        items.forEach(item => {
          html += \`
            <div class="bargain-item">
              <strong>\${item.name}</strong><br>
              Price: \${item.price} | Discount: \${item.discount}% below average<br>
    ...
    container.innerHTML = hasBargains ? html : '<p style="color:#999;">No bargains found yet. Keep scraping!</p>';
  }
  ...
  document.getElementById('addBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      if (res.ok) {
        addLog(\`➕ Added term: "\${term}" (avg: £\${averagePrice || 'auto'}, deal: \${thresholdPercent}%)\`, 'success');
        termInput.value = '';
        fetchData();
      } else {
...
app.get('/status', (req, res) => {
  const termStatus = searchTerms.map(termObj => {
    ...
    const unique = loadUniqueFile(term);
...
wss.on('connection', (ws) => {
  ...
  // Check if this is a frontend client (browser) or worker
  ws.isFrontend = false;
    ...
    try {
      ...
      switch (data.type) {
        ...
        case 'register-worker': {
          clientId = data.clientId || `worker-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`;
          ...
          processQueue(); // Check if there are jobs waiting
          broadcastUpdate();
          break;
        }
...
setInterval(() => {
  const now = Date.now();
  for (let [id, info] of clients) {
    if (now - info.lastPing > 30000) { // 30 seconds without ping
      console.log(`⚠️ Worker ${id} stale, closing connection`);
      info.ws.terminate();
      clients.delete(id);
    }
  }
...
// ─── Job scheduler ──────────────────────────────────────────────────
// Run every minute to check for jobs that need to be scheduled
setInterval(() => {
  for (const termObj of searchTerms) {
    const term = termObj.term;
    const interval = termObj.interval || 5;
    
    // Check if this term should be scheduled
    if (!activeJobs.has(term) && !jobQueue.includes(term)) {
